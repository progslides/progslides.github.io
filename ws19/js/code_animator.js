...
function animateCodeStep(block, dir) {
  var step = parseInt(document.getElementById("ca_" + block + "_step").value);
  var max_step = parseInt(document.getElementById("ca_" + block + "_max_step").value);
  
  // reset current line
  if(step < max_step && step > 0) {
    if(!(step == max_step - 1 && dir == 1)) {
      var c_line = document.getElementById("ca_" + block + "_" + step.toString(36));
      if(c_line != null) c_line.className = "";
      var c_block = document.getElementById("ca_" + block + "_stat_" + step.toString(36));
      if(c_block != null) c_block.style.display = "none";
    }
  }
  
  // highlight current line
  if(step >= 1 && step < max_step) step_n = step + dir;
  step = step_n;
  if(step == 0) step = 1;
  if(step < max_step) {
    var c_line = document.getElementById("ca_" + block + "_" + step.toString(36));
